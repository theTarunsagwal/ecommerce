...
<html lang="en">
  ...
  $con = mysqli_connect('localhost','root','','ecommerce');
    ...
    $login_counts_query = mysqli_query($con, "
   SELECT DATE(last_login) as login_day, COUNT(*) as login_count 
    FROM user_data
...
$login_data = [];
while ($row = mysqli_fetch_assoc($login_counts_query)) {
    $login_data[] = $row;
}

$login_days = array_column($login_data, 'login_day');
$login_counts = array_column($login_data, 'login_count');
?>
    ...
    <div class="card">
        <div class="card-body">
            <h4 class="card-title">User Logins Per Day</h4>
    ...
    <script>
        ...
        var loginDays = <?= json_encode($login_days) ?>;
        var loginCounts = <?= json_encode($login_counts) ?>;
